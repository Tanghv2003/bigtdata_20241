...
spark = SparkSession.builder \
    .appName("3 worker * 1 executor * 2 core - 1 partition") \
...
try:
    df = spark.read.option("header", "true").csv(hdfs_file_path).repartition(1)
...
try:
    delay_analysis = df.repartition(1).groupBy("OP_CARRIER").agg(
...
try:
    time_analysis = df.repartition(1).groupBy(
...
try:
    df_distance = df.withColumn("DISTANCE_CATEGORY", 
        ...
        .otherwise("Long Haul")).repartition(1)
...
try:
    delay_severity = df.select(
        ...
        "OP_CARRIER"
    ).repartition(1).groupBy("OP_CARRIER", "DELAY_CATEGORY").agg(
        count("*").alias("flight_count")
    ).repartition(1).cache()
...
try:
    performance_metrics = df_clean.repartition(1).agg(
        count("*").alias("total_flights"),
