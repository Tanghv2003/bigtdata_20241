...
class FlightKafkaProducer:
    def __init__(self, kafka_config, topic):
        ...
        self.producer = KafkaProducer(
            ...
            value_serializer=lambda v: json.dumps(v).encode('utf-8')  # Chuyển đổi dữ liệu sang JSON
        )
        self.topic = topic
...
def main():
    ...
    begin_time = end_time - 2 * 3600  # Lấy dữ liệu trong 2 giờ qua
    
    print("\nFetching global flight data...")
    data = processor.get_flights(begin_time, end_time)
    
    # Nếu có dữ liệu, gửi dữ liệu tới Kafka và hiển thị trên màn hình
    if not data.empty:
        # Cấu hình Kafka
        kafka_config = {
            'bootstrap.servers': 'kafka:29092',  # Địa chỉ Kafka broker
        }
        kafka_producer = FlightKafkaProducer(kafka_config, 'flights')  # Kafka topic là 'flights'
        
        # Gửi từng dòng dữ liệu trong DataFrame tới Kafka
        for _, flight_data in data.iterrows():
            flight_json = flight_data.to_dict()  # Chuyển đổi dữ liệu thành dict
            kafka_producer.send_flight_data(flight_json)
            print(flight_json)  # In dữ liệu flight lên màn hình
    
    processor.print_flights()  # In ra dữ liệu
    processor.export_to_csv('global_flights.csv')  # Lưu dữ liệu vào file CSV
